...
No* insere(No* lista, int num,int element){
  ...
  // Se a lista estiver vazia, lista aponta para novo.
  if(lista == NULL){
      lista = novo;
    }
...
// Segunda função é chamada para tratar a colisão.
int hashTwo(int tam, int vetor){
  return (vetor + 1) % tam;
  }
// Primeira função é chamada para buscar o indice a ser inserido.
int hashOne(int tam, int vetor){
  return vetor % tam;
  }
  
// Alocando o valore em suas posições.
...
int main() {
  ...
  while(i < tam){
    ...
    i++;
  	...
  	// chamando a tabela hash e insere na lista.
  while(count < tam){
